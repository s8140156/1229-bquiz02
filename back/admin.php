<fieldset>
	...
	<legend>帳號管理</legend>
	<form action="./api/edit_user.php" method="post"> <!--增加form注意路徑-->
		<table style="width:55%;margin:auto;text-align:center"> <!--css美化-->
			<tr>
				<td class="clo">帳號</td>
				<td class="clo">密碼</td>
				<td class="clo">刪除</td>
			...
			$rows=$User->All();
			foreach($rows as $row){
				if($row['acc']!=='admin'){ //這邊判斷如果資料庫裡只要不是admin 就show出來 讓admin帳號不要show在上面

					
					?>
			<tr>
				<td><?=$row['acc'];?></td>
				<td><?=str_repeat("*",mb_strlen($row['pw']));?></td> <!-- 密碼不要顯示 使用計算密碼字串然後用*取代 -->
